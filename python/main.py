...
class ReplayBuffer(object):
    ...
    def Clear(self):
        self.buffer.clear()
class PPO(object):
    def __init__(self,meta_file,exp_name):
    ...
    def SaveModel(self):
        self.model.save('../nn/{}/current.pt'.format(self.exp_name))
        self.muscle_model.save('../nn/{}/current_muscle.pt'.format(self.exp_name))

        if self.max_return_epoch == self.num_evaluation:
            self.model.save('../nn/{}/max.pt'.format(self.exp_name))
            self.muscle_model.save('../nn/{}/max_muscle.pt'.format(self.exp_name))
        if self.num_evaluation%100 == 0:
            self.model.save('../nn/{}/{}.pt'.format(self.exp_name, str(self.num_evaluation//100)))
            self.muscle_model.save('../nn/{}/{}_muscle.pt'.format(self.exp_name, str(self.num_evaluation//100)))
...
if __name__=="__main__":
    ...
    parser.add_argument('-n','--name', help='experiment name', default='default')
    ...
    ppo = PPO(args.meta, args.name)
    nn_dir = '../nn'
    if not os.path.exists(nn_dir):
        os.makedirs(nn_dir)
    nn_exp_dir = '../nn/' + args.name
    if not os.path.exists(nn_exp_dir):
        os.makedirs(nn_exp_dir)
